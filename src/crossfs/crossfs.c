 ...
 */
static inline int is_equal_or_parent(const char *const a,
				     const size_t a_len, const char *b,
				     const size_t b_len)
{
    if (a_len > b_len) {
	return 0;
 ...
 */
static inline char *calc_bpath(struct cfg_entry *cfg,
			       struct back_entry *back, const char *ipath,
			       size_t ipath_len, char tmp[PATH_MAX])
{
 ...
 */
static inline int insert_h_str(struct h_str **strs, char *str,
			       size_t str_len)
{
    struct h_str *e = NULL;
 ...
 */
static inline int fchroot_stat(int root_fd, const char *bpath,
			       struct stat *buf)
{
    int rv = -EAGAIN;
    pthread_mutex_lock(&root_lock);
 ...
 */
static inline int fchroot_readlink(int root_fd, const char *bpath,
 ...
 * hit.
 */
static inline int stat_first_bpath(struct cfg_entry *cfg,
				   const char *ipath, size_t ipath_len,
				   struct stat *stbuf)
{
    int rv = -ENOENT;
 ...
 * hit.
 */
static inline int open_first_bpath(struct cfg_entry *entry,
				   const char *ipath, size_t ipath_len,
				   int flags)
{
    int rv = -ENOENT;
				  ...
				  char obpath[PATH_MAX])
{
	...
	char c;
	rv = fchroot_readlink(deref(&cfg->back[i])->root_fd, bpath, &c, 1);
	if (rv >= 0 || errno != ENOENT) {
	    *back = &cfg->back[i];
	    obpath[PATH_MAX - 1] = '\0';
 ...
 */
static inline int filldir_all_bpath(struct cfg_entry *cfg,
				    const char *ipath, size_t ipath_len,
				    struct h_str *files)
{
    int rv = 0;
	...
	rv |= fchroot_filldir(deref(&cfg->back[i])->root_fd, bpath, files);
    }
    return rv;
}
			       ...
			       size_t ipath_len, struct stat *stbuf)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		 ...
		 * this should be safe.
		 */
		if (memcmp(line, ini_exec_str[i], ini_exec_len[i]) != 0) {
		    continue;
		}
...
static inline int read_pass(struct cfg_entry *cfg, const char *const ipath,
			    size_t ipath_len, char *buf, size_t size,
			    off_t offset)
{
    int rv;
...
static inline int read_back(struct cfg_entry *cfg, const char *ipath,
			    size_t ipath_len, char *buf, size_t size,
			    off_t offset)
{
    int rv;
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
	    int found = 0;
	    for (size_t i = 0; i < ARRAY_LEN(ini_exec_str); i++) {
		if (strncmp(line, ini_exec_str[i], ini_exec_len[i]) != 0) {
		    continue;
		}
		...
		strcatoff(buf, deref(back)->name,
			  deref(back)->name_len, &off, &wrote, size);
		strcatoff(buf, " ", 1, &off, &wrote, size);
